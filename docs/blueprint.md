...
- Budget Impact Transactions: Full CRUD operations for transactions that directly impact the budget. Income transactions add to the budget (and optionally to balance), while Withdraw transactions subtract from the budget (and optionally from balance).
